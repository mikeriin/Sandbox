...
namespace mk
{
	...
	inline uint32_t PackTypeUVExtentAndAO(uint8_t type, uint8_t u, uint8_t v, uint8_t ao);
	...
	inline uint8_t GetVoxelBlockType(const mk::Voxel& vox);
	...
	inline VoxelFace CreateFace(const glm::ivec3& pos, mk::FaceDirection face, const glm::ivec2 extent, const mk::BlockType& type);
}
...
uint32_t mk::PackTypeUVExtentAndAO(uint8_t type, uint8_t u, uint8_t v, uint8_t ao)
{
	return (type & MK_MASK_8_BITS) << MK_VOXEL_BLOCK_TYPE_SHIFT |
		(u & MK_MASK_6_BITS) << MK_VOXEL_U_SHIFT |
...
uint8_t mk::GetVoxelBlockType(const mk::Voxel& vox)
{
	return (vox.packedB >> MK_VOXEL_BLOCK_TYPE_SHIFT) & MK_MASK_8_BITS;
}
...
mk::VoxelFace mk::CreateFace(const glm::ivec3& pos, mk::FaceDirection norm, const glm::ivec2 extent, const mk::BlockType& type)
{
	mk::VoxelFace face;
	...
	case mk::Front:
	default:
		face[0] = { PackPositionAndNormal(pos.x						, pos.y + extent.y, pos.z + 1	, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, extent.y, 0) };
		face[1] = { PackPositionAndNormal(pos.x + extent.x, pos.y + extent.y, pos.z + 1	, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, extent.y, 0) };
		face[2] = { PackPositionAndNormal(pos.x + extent.x, pos.y						, pos.z + 1	, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, 0				, 0) };
		face[3] = { PackPositionAndNormal(pos.x						, pos.y						, pos.z + 1	, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, 0				, 0) };
		break;
	case mk::Back:
		face[0] = { PackPositionAndNormal(pos.x + extent.x, pos.y + extent.y, pos.z			, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, extent.y, 0) };
		face[1] = { PackPositionAndNormal(pos.x						, pos.y + extent.y, pos.z			, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, extent.y, 0) };
		face[2] = { PackPositionAndNormal(pos.x						, pos.y						, pos.z			, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, 0				, 0) };
		face[3] = { PackPositionAndNormal(pos.x + extent.x, pos.y						, pos.z			, norm),
			PackTypeUVExtentAndAO((uint8_t)type, 0				, 0				, 0) };
		break;
	case mk::Right:
		face[0] = { PackPositionAndNormal(pos.x + 1	, pos.y + extent.y, pos.z + extent.x, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, extent.y, 0) };
		face[1] = { PackPositionAndNormal(pos.x + 1	, pos.y + extent.y, pos.z						, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, extent.y, 0) };
		face[2] = { PackPositionAndNormal(pos.x + 1	, pos.y						, pos.z						, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, 0				, 0) };
		face[3] = { PackPositionAndNormal(pos.x + 1	, pos.y						, pos.z + extent.x, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, 0				, 0) };
		break;
	case mk::Left:
		face[0] = { PackPositionAndNormal(pos.x			, pos.y + extent.y, pos.z						, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, extent.y, 0) };
		face[1] = { PackPositionAndNormal(pos.x			, pos.y + extent.y, pos.z + extent.x, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, extent.y, 0) };
		face[2] = { PackPositionAndNormal(pos.x			, pos.y						, pos.z + extent.x, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, 0				, 0) };
		face[3] = { PackPositionAndNormal(pos.x			, pos.y						, pos.z						, norm),
			PackTypeUVExtentAndAO((uint8_t)type, 0				, 0				, 0) };
		break;
	case mk::Top:
		face[0] = { PackPositionAndNormal(pos.x						, pos.y + 1	, pos.z						, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, extent.y, 0) };
		face[1] = { PackPositionAndNormal(pos.x + extent.x, pos.y + 1	, pos.z						, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, extent.y, 0) };
		face[2] = { PackPositionAndNormal(pos.x + extent.x, pos.y + 1	, pos.z + extent.y, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, 0				, 0) };
		face[3] = { PackPositionAndNormal(pos.x						, pos.y + 1	, pos.z + extent.y, norm),
			PackTypeUVExtentAndAO((uint8_t)type, 0				, 0				, 0) };
		break;
	case mk::Bottom:
		face[0] = { PackPositionAndNormal(pos.x						, pos.y			, pos.z + extent.y, norm), 
			PackTypeUVExtentAndAO((uint8_t)type, 0				, extent.y, 0) };
		face[1] = { PackPositionAndNormal(pos.x + extent.x, pos.y			, pos.z + extent.y, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, extent.y, 0) };
		face[2] = { PackPositionAndNormal(pos.x + extent.x, pos.y			, pos.z						, norm),
			PackTypeUVExtentAndAO((uint8_t)type, extent.x	, 0				, 0) };
		face[3] = { PackPositionAndNormal(pos.x						, pos.y			, pos.z						, norm),
			PackTypeUVExtentAndAO((uint8_t)type, 0				, 0				, 0) };
		break;
